...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    # [NOTE] While inverting sigmas can be done with setting 'scheduler.config.invert_sigmas' to True,
    #        we just flip the timesteps here.
    if inversion:
        scheduler.timesteps = timesteps.flip(0)
        scheduler.sigmas = scheduler.sigmas.flip(0)
        timesteps = scheduler.timesteps
...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    ...
    def inversion(
        ...
        images: Optional[Union[torch.FloatTensor, PIL.Image.Image, np.ndarray]] = None,
    ):
        r"""
        Function invoked when calling the pipeline for generation.
        ...
        if image_latents is not None:
            # [NOTE] here, assume image_latents are not scaled & shifted yet
            latents = image_latents.to(self.vae.device, self.vae.dtype)
            ...
            images = images.to(self.vae.device, self.vae.dtype)
            latents = self.vae.encode(images, return_dict=False)[0].mode()
    ...
    def rf_solver(
        ...
        """
        if image_latents is not None:
            # [NOTE] here, assume image_latents are not scaled & shifted
            latents = image_latents.to(self.vae.device, self.vae.dtype)
            latents = self.vae.config.scaling_factor * (latents - self.vae.config.shift_factor)
        elif images is not None:
            images = self.image_processor.preprocess(images)
        ...
        # [NOTE] timestep: [t_1, t_2, ..., t_N] -> [t_0, t_1, t_2, ..., t_N]
        timesteps = torch.cat([torch.FloatTensor([0]).to(timesteps.device), timesteps])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:])):
                ...
                latent_model_input = torch.cat([latents] * 2) if self.do_classifier_free_guidance else latents
                # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
                timestep = t_prev if inversion else t_curr
                ...
                dt = 0.5 * (sigma_next - sigma)

                latents_mid = latents + dt * noise_pred
                latent_model_input_mid = torch.cat([latents_mid] * 2) if self.do_classifier_free_guidance else latents_mid
                timestep_mid = timestep + 0.5 * (t_prev - t_curr)

                noise_pred_mid = self.transformer(
                    hidden_states=latent_model_input_mid,
                ...
                if self.do_classifier_free_guidance:
                    ...
                    noise_pred_mid = noise_pred_mid_uncond + self.guidance_scale * (noise_pred_mid_text - noise_pred_mid_uncond)

                # compute the previous noisy sample x_t -> x_t-1
                latents_dtype = latents.dtype
                
                first_order = (noise_pred_mid - noise_pred) / dt
                latents = latents + (sigma_next - sigma) * noise_pred + 0.5 * (sigma_next - sigma) ** 2 * first_order

                if inversion and log_inversion:
                    #===== visualize inversion =====#
                    latents_vis = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor

                    image = self.vae.decode(latents_vis, return_dict=False)[0]
                    image = self.image_processor.postprocess(image, output_type='pil')[0]
                    image.save(f'inversion_log/vanilla/inversed_t{int(t_prev.item())}.png')
                    #===== visualize inversion =====#
